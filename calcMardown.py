...
file_n=input("doc name > ")
try:
  f=__import__(file_n)
  f_valid=True
except (ImportError, SyntaxError) as e:
  if isinstance(e,SyntaxError):
    print("Python syntax error in file")
  else:
    print("file not found!")
  f_valid=False

if f_valid and not "doc" in dir(f):
  print("invalid file!")
  f_valid=False
...
if f_valid:
  ...
  try:
    ...
    while True:
      ...
      total_height=0
      for i in text.split("\n"):
        height=parse_md_line(i, y_level)
        y_level += height
        total_height+=height

      kandinsky.fill_rect(313,9,5,203,(200,200,200))
      kandinsky.fill_rect(314,10+int(201*(-y_scroll)/total_height),3,round(201*(222/total_height)),(50,50,50))

      kd=False
      ku=False
      while not (ku or kd):
        kd=ion.keydown(ion.KEY_DOWN)
        ku=ion.keydown(ion.KEY_UP)
        time.sleep(0.1)
      if kd:
        y_scroll-=50
      if ku:
        y_scroll+=50
      y_scroll=min(y_scroll,0)
      y_scroll=max(y_scroll,-total_height+222)
    
      # clear
      kandinsky.fill_rect(0,0,312,222,(255,255,255))
  except KeyboardInterrupt:
    kandinsky.fill_rect(0,0,320,222,(255,255,255))
    kandinsky.draw_string("Press [OK] or [Back] to close",10,10)
    print("goodbye!")
